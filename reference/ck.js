...
try {
    ...
    if (paramPrescoIdArray == '') {
       if (ck.hasItem('CPR' + cid)) {
          // P_CPR = Parameter_ContentId_Presco の略
          paramPrescoid = ck.getItem('P_CPR' + cid) + '.' + ck.getItem('CPR' + cid);
          // ドット区切りでパラメータを分割する
          paramPrescoIdArray = paramPrescoid.split('.');
          // バリデーションの実施
       ...
       } else if (ck.hasItem('GPR' + gid)) {
          // P_GPR = Parameter_GroupId_Presco の略
          paramPrescoid = ck.getItem('P_GPR' + gid) + '.' + ck.getItem('GPR' + gid);
          // ドット区切りでパラメータを分割する
          paramPrescoIdArray = paramPrescoid.split('.');
          // バリデーションの実施
...
function PrescoGetDomain()
{
    ...
    if (currentScriptElement) {
       currentScript = currentScriptElement.src;
        ...
        currentScript = location.protocol + '//ad.presco.asia/fpc/ck\.js';
    }
